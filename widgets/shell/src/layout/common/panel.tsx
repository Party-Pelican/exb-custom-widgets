...
import { ItemTitle } from "./layout-title";
...
export function Panel(props: LayoutItemProps & OwnProps) {
  ...
  const handleResizeStart = React.useCallback(
    (id: string, initWidth: number, initHeight: number) => {
  ...
  const handleResizing = React.useCallback(
    (id: string, x: number, y: number, dw: number, dh: number) => {
  ...
  const width = isResizing
    ? sizeRef.current.width + deltaSize.w
    : sizeRef.current.width;
  const height = isResizing
    ? sizeRef.current.height + deltaSize.h
    : sizeRef.current.height;
  return (
    <div
      className={classNames("exb-rnd floating-panel shadow-lg", {
        "d-none": isMinimized,
        active: isActive,
        shadow: isActive,
        maximize: mode === Modes.Maximum,
      })}
      css={css`
        ${style};
        top: ${bbox.top};
      ...
      <div className="panel-header d-flex align-items-center px-1">
        <div className="tool-drag-handler">
          {layoutId && layoutItemId && (
            <ItemTitle layoutId={layoutId} layoutItemId={layoutItemId} />
          )}
        </div>
      ...
      >
        ...
        <div
          className={classNames(
            "resize-handler right rnd-resize-bottom rnd-resize-right",
            { "d-none": collapsed || mode !== Modes.Normal }
          )}
        >
          <ShapeL pos="br" />
        </div>
